...
> 一句话：**dev 版 X.Y（Y≥1）做迭代，每轮 +0.1；用户确认后转正为同主版本的 stable X.0；下一轮 dev 从 (X+1).1 开始。**
...
```
dev 1.1 ── +0.1 ──▶ dev 1.2 ── +0.1 ──▶ dev 1.3 ── 用户确认转正 ──▶ stable 1.0
                                                                            │
                                                                            ▼ 下一轮
                                              dev 2.1 ── +0.1 ──▶ dev 2.2 ── 确认 ──▶ stable 2.0 ──▶ …
```

- 迭代：`dev X.Y → X.(Y+1)`（注意 1.9 → 1.10，不是 2.0；整数加法，不用浮点）。
- 转正：`dev X.Y → stable X.0`（**同主版本**，次版本归零）。
- 新一轮：`stable X.0 → dev (X+1).1`（主版本 +1，次版本从 1 开始）。
