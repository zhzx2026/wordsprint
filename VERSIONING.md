...
> 一句话：**dev 版 X.Y（Y≥1）做迭代，每轮 +0.1；用户确认后转正为 stable X.0（该号已发布过就顺延到下一个）；下一轮 dev 从同一主版本的 X.1 开始。**
...
| 稳定版 stable | `主版本.0` | 1.0、2.0、3.0 | dev 经用户确认后转正 |
...
dev 1.1 ── +0.1 ──▶ dev 1.2 ── +0.1 ──▶ dev 1.3 ── 用户确认转正 ──▶ stable 2.0
                                                                            │
                                                                            ▼ 下一轮（同一主版本）
                                              dev 2.1 ── +0.1 ──▶ dev 2.2 ── 确认 ──▶ stable 3.0 ──▶ …
```

- 迭代：`dev X.Y → X.(Y+1)`（注意 1.9 → 1.10，不是 2.0；整数加法，不用浮点）。
- 转正：`dev X.Y → stable X.0`；**若 `vX.0` 早已发布过**（比如当前 stable 就是 2.0，而 dev 也是 2.x），
  自动顺延到 `(X+1).0` —— 版本名不重复（`version.sh promote` 里查远端 tag 自动判断）。
- 新一轮：`stable X.0 → dev X.1`（**主版本不变**，次版本从 1 开始）。
  例：stable 2.0 之后是 dev 2.1（而不是 3.1）—— 2026-09-14 用户纠正，`version.sh bump-dev` 已按此改。

> 历史更正：本文件与 `version.sh` 最初把「新一轮」写成 `(X+1).1`（stable 2.0 → dev 3.1），
> 2026-09-14 用户指出应为 `X.1`（stable 2.0 → dev 2.1）。已同步改脚本、本文件与 AGENT.md；
> 当时误生成的 dev 3.1（code 21）用 `bash scripts/version.sh set 2.1 22` 拉回。
...
- 转正 = `version.sh promote`（X.Y→X.0）→ 提交 → 合 PR → CI 自动打 tag `vX.0` 并发 Release → 手机 OTA。
- tag 只打给 stable（`v1.0`、`v2.0`…）；dev 版本永远没有 tag（staging 不建 tag）。
...
bash scripts/version.sh promote    # 用户确认转正时跑：X.Y→X.0（只改文件，再走合 PR 流程）
...
## 8. 迁移说明（v1.0.17 → v2.1/dev，用户 2026-09-13 选定）

- 2026-09-13 前的 `1.0.x`（含已发布的 v1.0.17/code 18）视为第 1 代，不再延续；旧 tag（`v1.0.8/9/14/17`）保留不动。
- 本次落地即执行第一轮迭代：`1.0.17（code 18）→ 2.1（code 19）`（`VERSION_NEW_MAJOR=2`，仅迁移这一次有效）。
- 首个新 stable 将是 `v2.0`（由 `2.x` dev 转正而来）；之后按状态机推进（2.0 → dev 3.1 …）。
