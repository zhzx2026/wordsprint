...
**当前版本：v9.0（stable）** <!-- CURRENT-VERSION -->
...
6. 进度实时续存：**每翻出一张卡就落盘一次**（组内剩余队列 + 回炉表 + 计数 + 用时，`Engine.snapshot/resume`），
   闪退 / 被系统杀后台 / 强行停止后重开，接着**那一张**继续而不是本组第一张；已记住的词永远跳过
...
- **5 套配色**（暖纸 / 松林 / 落日 / 莓红 / 深海）× 跟随系统 / 浅色 / 深色，设置里实时切换（深色是 navy 系）；
  换配色/深浅/字体/字号后返回其他页面立即生效，不用重启 App（外观代数对账，见 `Look.java`）
...
- 设置 →「数据与档案」→ **导出二维码**：内容**可选**（词书进度 / 错题本 / 打卡记录 / 学习设置四枚开关 +
  逐本勾选，改完当场重算）。一枚码里有：各书掌握位图 + 组指针 + **错题本**（每词还差几次）+
  **完整打卡日记**（目标/温习/用时/完成勾）+ **学习设置**（手势/目标/分组）+ 档案名
  （自定义二进制 → Deflate → Base64URL → 自研 QR 编码器 `QREnc.java`，EC-M / v1–40；
  每张图生成前用与扫码端同一套 zxing **自解码校验**，读不通就换掩码，保证「扫得到」；
  扩展段附在 v1 字节后面，**老版本 App 读到原来位置就停**，新码旧机照扫不报错）
- 内容太多二维码装不下（上限 2331 字节）不再闪退：码图区换成提示，**文本码照样可用**
  （粘贴导入不限长度），或少选几本分多次传；生成放子线程，连点开关只有最后一次生效
- 另一台手机：同页「扫码导入」→ 对准二维码，振动确认后**合并**（掌握词取并集、进度取更大、
  打卡取更大、错题取并集且「还差几次」取大、日记计数取大/目标取大，不丢数据）；
  成功页列明细 + “来自谁”；对方设置有差异可**一键采用**（默认不动本机任何设置）
