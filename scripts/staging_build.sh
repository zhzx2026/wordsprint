...
#   · dev 通道按坑位发布（channels/<分支id>/），不覆盖别人的坑；
...
echo "手机装机（开发者通道 · 本分支坑位）：设置 → 更新源填
  https://raw.githubusercontent.com/$(git remote get-url origin | sed -E 's#(https://|git@)(github\.com[:/])##; s#\.git$##')/dev/channels/$BID
  → 检查 → 立即更新（覆盖安装，进度保留）。" )
